...
MCQ evaluation pipeline for Bengali riddles using Google Gemini 2.5 Flash model.
...
DATASET_PATH = Path("/mnt/wwn-0x500a0751e14d807e-part2/Labib/Labib Folder/Labib/Research/BornoDhada/main_dataset/riddles_mcq.json")
OUTPUT_ROOT = Path("/mnt/wwn-0x500a0751e14d807e-part2/Labib/Labib Folder/Labib/Research/BornoDhada/Code/Model Evaluation (Benchmark)/discriminative evaluation/Gemini-flash")
...
def process_mcq_evaluation(mcq_data, gemini_client, prompt_mode="zero_shot"):
    """Process MCQ evaluation with specified prompt mode."""
    
    out_file = OUTPUT_ROOT / f"riddles_mcq_gemini_flash_{prompt_mode}.json"
    ...
    if not remaining_data:
        ...
        metrics_out = OUTPUT_ROOT / f"riddles_mcq_metrics_gemini_flash_{prompt_mode}.json"
    ...
    for item in tqdm(remaining_data, desc=f"MCQ Evaluation ({prompt_mode})"):
        riddle_id = item.get("id")
        riddle_text = item.get("question", "")  # Use 'question' field
        options = item.get("options", [])
        correct_answer = item.get("correct_answer", "")
        
        if not riddle_text or not options:
            print(f"⚠️ Skipping item {riddle_id}: missing riddle or options")
            continue
        
        # Find ground truth index
        ground_truth_index = None
        for i, option in enumerate(options):
            if normalize_text(option) == normalize_text(correct_answer):
        ...
        # Create prompt
        prompt = prompt_template.format(riddle=riddle_text, options=options_str)
...
def main():
    """Main function to run MCQ evaluation."""
    print("🎯 Bengali Riddle MCQ Evaluation - Gemini 2.5 Flash")
    print("=" * 60)
    
    # Load data
    print(f"📂 Loading MCQ data from: {DATASET_PATH}")
