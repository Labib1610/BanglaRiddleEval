...
Semantic Ambiguity evaluation pipeline for Bengali riddles using Ollama gemma3:12b model.
...
OUTPUT_ROOT = Path("/mnt/wwn-0x500a0751e14d807e-part2/Labib/Labib Folder/Labib/Research/BornoDhada/Code/Model Evaluation (Benchmark)/classification evaluation/Gemma/gemma3:12b")
OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)

MCQ_DATASET_PATH = Path("/mnt/wwn-0x500a0751e14d807e-part2/Labib/Labib Folder/Labib/Research/BornoDhada/dataset/riddles_semantic_ambiguity.json")
...
def extract_reasoning_and_final_answer(answer_text):
    ...
    if m_final_idx:
        ...
        # 4) For incomplete responses, try to extract any number that could be an index
        if idx is None:
            # Look for any standalone number in the range 0-3 (typical for 4 options)
            numbers = re.findall(r'\b([0-3])\b', text)
            if numbers:
                idx = int(numbers[-1])  # Take the last valid index found
...
def process_mcq_evaluation(ollama_llm: OllamaLLM, prompt_mode="few", n_examples=None):
    print(f"\n==== Processing Semantic Ambiguity evaluation (prompt_mode={prompt_mode}) ====")

    mcq_data = load_mcq_data(MCQ_DATASET_PATH)
    
    if not mcq_data:
        print(f"⚠️ No semantic ambiguity data found in {MCQ_DATASET_PATH}")
        return
    
    # Process all data or limit by n_examples if specified
    if n_examples is not None:
        mcq_data = mcq_data[:n_examples]
        print(f"📊 Using first {len(mcq_data)} items (n_examples={n_examples})")
    else:
        print(f"📊 Processing all {len(mcq_data)} items")

    if not mcq_data:
        print("⚠️ No MCQ data available")
        return

    out_file = OUTPUT_ROOT / f"riddles_semantic_ambiguity_gemma3_12b_{prompt_mode}.json"
    ...
    if not examples_to_process:
        ...
        metrics_out = OUTPUT_ROOT / f"riddles_semantic_ambiguity_metrics_gemma3_12b_{prompt_mode}.json"
    ...
    for item in tqdm(examples_to_process, desc=f"Semantic Ambiguity Evaluation"):
        riddle_id = item.get("id")  # Correct field name from dataset
        riddle = item.get("riddle")  # Original riddle text
        ans = item.get("ans")  # Correct field name from dataset
        question = item.get("question")  # Semantic ambiguity question
        options = item.get("options", [])
        correct_answer = item.get("correct_option")  # ground truth answer string
        ...
        # Set higher token limit for CoT mode to allow complete reasoning
        max_tokens = 2048 if prompt_mode == "cot" else None
        
        for attempt in range(1, max_attempts + 1):
            try:
                raw_resp, resp_preview = ollama_llm.generate(full_prompt, max_tokens=max_tokens)
                break
            except Exception as e:
        ...
        if prompt_mode == "cot":
            ...
            # For incomplete CoT responses, preserve the partial reasoning
            if reasoning_text is None and "Reasoning_En:" in answer_text_raw:
                reasoning_text = answer_text_raw.strip()
                # Extract partial steps if available
                if not reasoning_steps:
                    step_matches = re.findall(r'Step\s*\d+\s*[:\-]?\s*([^\\n]+)', answer_text_raw, flags=re.IGNORECASE)
                    if step_matches:
        ...
        result_item = {
            ...
            "semantic_question": normalize_text(question),
        ...
        # Longer sleep for CoT mode to allow model time to generate complete responses
        sleep_time = 15 if prompt_mode == "cot" else 10
...
def main():
    ...
    for prompt_mode in ["zero", "few", "cot"]:
        ...
        try:
            ...
            print(f"❗ Error processing Semantic Ambiguity evaluation ({prompt_mode}-shot): {e}")
