...
Semantic Ambiguity evaluation pipeline for Bengali riddles using Ollama qwen3:14b model.
...
OUTPUT_ROOT = Path("/mnt/wwn-0x500a0751e14d807e-part2/Labib/Labib Folder/Labib/Research/BornoDhada/Code/Model Evaluation (Benchmark)/classification evaluation/Qwen3/qwen3:14b")
...
def process_mcq_evaluation(ollama_llm: OllamaLLM, prompt_mode="few", n_examples=None):
    ...
    # Use specified number of examples or entire dataset
    if n_examples is not None:
        mcq_data = mcq_data[:n_examples]
        print(f"📊 Using first {len(mcq_data)} items (n_examples={n_examples})")
    else:
        print(f"📊 Using entire dataset with {len(mcq_data)} items")

    if not mcq_data:
        print("⚠️ No MCQ data available after sampling")
        return

    out_file = OUTPUT_ROOT / f"riddles_semantic_ambiguity_qwen3_14b_{prompt_mode}.json"
    ...
    if not examples_to_process:
        ...
        metrics_out = OUTPUT_ROOT / f"riddles_semantic_ambiguity_metrics_qwen3_14b_{prompt_mode}.json"
