# Gemma3:4b Reasoning Evaluation Suite

This directory contains reasoning evaluation pipeline for testing the Gemma3:4b model on Bengali riddle reasoning generation.
...
- `gemma3_4b_generative.py` - Reasoning evaluation script with LLM judge
...
**Important**: Add your Google Gemini API keys to the `KEY_LIST` in `gemma3_4b_generative.py`:
```python
KEY_LIST = [
    "your-api-key-1",
    "your-api-key-2",
...
cd "/path/to/gemma3:4b"

# Run reasoning evaluation (with LLM judge and LLM Judge)
python run_generative_evaluation.py

# Or run directly
python gemma3_4b_generative.py
```

### Configuration

Edit the CONFIG section in `gemma3_4b_generative.py`:
...
- `riddles_reasoning_gemma3_4b_zero_shot_full_dataset.json` - Zero-shot results
- `riddles_reasoning_gemma3_4b_few_shot_full_dataset.json` - Few-shot results  
- `riddles_reasoning_gemma3_4b_chain_of_thought_full_dataset.json` - CoT results
- `riddles_reasoning_metrics_gemma3_4b_zero_shot_full_dataset.json` - Zero-shot metrics
- `riddles_reasoning_metrics_gemma3_4b_few_shot_full_dataset.json` - Few-shot metrics
- `riddles_reasoning_metrics_gemma3_4b_chain_of_thought_full_dataset.json` - CoT metrics
...
### 1. LLM-as-a-Judge (Gemini 2.5 Flash)
...
- Processes entire reasoning dataset for comprehensive evaluation
- Consistent evaluation across all three prompting modes