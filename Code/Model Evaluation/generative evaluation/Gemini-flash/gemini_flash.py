...
Generative evaluation pipeline for Bengali riddles using Google Gemini-2.5-Flash model.
- Evaluates open-ended riddle answer generation
...
# Note: Using Google Gemini API only - no Ollama needed
...
# Google Gemini API Configuration - Split keys for evaluation and judging
...
OUTPUT_ROOT = Path("/mnt/wwn-0x500a0751e14d807e-part2/Labib/Labib Folder/Labib/Research/BornoDhada/Code/Model Evaluation (Benchmark)/generative evaluation/Gemini-flash")
        ...
        while attempt < max_attempts:
            ...
            try:
                ...
                resp = model.generate_content(
                    prompt,
                    generation_config=genai.types.GenerationConfig(
                        temperature=0.3,  # Slightly higher temperature for creative answers
                        top_k=40,
                        top_p=0.95,
...
def process_generative_evaluation(gemini_llm: RotatingGeminiEvaluationClient, gemini_judge: RotatingGeminiClient, prompt_mode="zero_shot"):
    ...
    out_file = OUTPUT_ROOT / f"riddles_generative_gemini_flash_{prompt_mode}.json"
    ...
    if not remaining_data:
        ...
        metrics = {
            ...
            "BERTScore F1 Average": round(avg_bert_f1, 3),
        ...
        metrics_out = OUTPUT_ROOT / f"riddles_generative_metrics_gemini_flash_{prompt_mode}.json"
    ...
    print(f"BERTScore F1 Average: {avg_bert_f1:.3f} ({avg_bert_f1 * 100:.2f}%)")

def main():
    """Main function to run generative evaluation."""
    print("🎯 Bengali Riddle Generative Evaluation - Gemini-2.5-Flash")
    print("=" * 60)
