#!/usr/bin/env python3
"""
deepseek_r1_7b_generative.py

Generative evaluation pipeline for Bengali riddles using Ollama deepseek-r1:7b model.
...
try:
    ...
    print("⚠️ bert_score package not found. Install it with:")
...
OUTPUT_ROOT = Path("/mnt/wwn-0x500a0751e14d807e-part2/Labib/Labib Folder/Labib/Research/BornoDhada/Code/Model Evaluation (Benchmark)/generative evaluation/Deepseek-r1/deepseek-r1:7b")
...
def compute_bert_score(predicted_list, ground_truth_list):
    """Compute BERTScore for a list of predictions and references."""
    ...
    try:
        # Use multilingual BERT model for Bengali support
        P, R, F1 = bert_score(predicted_list, ground_truth_list, 
...
def process_generative_evaluation(ollama_llm: OllamaLLM, gemini_judge: RotatingGeminiClient, prompt_mode="zero_shot"):
    ...
    out_file = OUTPUT_ROOT / f"riddles_generative_deepseek_r1_7b_{prompt_mode}.json"
    ...
    if not remaining_data:
        ...
        metrics = {
            ...
            "LLM Judge Accuracy (%)": round(avg_llm_judge_score * 100, 2),
        ...
        metrics_out = OUTPUT_ROOT / f"riddles_generative_metrics_deepseek_r1_7b_{prompt_mode}.json"
        with open(metrics_out, "w", encoding="utf8") as f:
            json.dump(metrics, f, ensure_ascii=False, indent=2)
        print(f"Metrics computed from existing results: LLM Judge Avg = {avg_llm_judge_score:.3f}, BERTScore F1 = {avg_bert_f1:.3f}")
        return
    ...
    for item in tqdm(remaining_data, desc=f"Generative Evaluation ({prompt_mode})"):
        ...
        # Create result (BERTScore will be computed in batch later)
        result = {
            "riddle_id": riddle_id,
            "riddle": riddle_text,
            "ground_truth": ground_truth,
            ...
            "bert_precision": 0.0,  # Will be updated after batch computation
            "bert_recall": 0.0,
            "bert_f1": 0.0
        }
    ...
    # After processing all items, compute BERTScore in batch for new results
    if remaining_data:
        print(f"\n🧠 Computing BERTScore for {len(remaining_data)} new results...")
        new_results = results[-len(remaining_data):]  # Get only the newly added results
        
        predicted_answers = [r.get("predicted_answer", "") for r in new_results]
        ground_truths = [r.get("ground_truth", "") for r in new_results] 
        
        bert_precisions, bert_recalls, bert_f1s = compute_bert_score(predicted_answers, ground_truths)
        
        # Update new results with BERTScore
        for i, result in enumerate(new_results):
            if i < len(bert_f1s):
                result["bert_precision"] = round(bert_precisions[i], 4)
                result["bert_recall"] = round(bert_recalls[i], 4)
                result["bert_f1"] = round(bert_f1s[i], 4)
        
        # Save final results with BERTScore
        with open(out_file, "w", encoding="utf8") as f:
            json.dump(results, f, ensure_ascii=False, indent=2)
    ...
    print(f"LLM Judge Average Score: {avg_llm_judge_score:.3f}/1.0 ({avg_llm_judge_score * 100:.1f}%)")
    print(f"Average BERTScore F1: {avg_bert_f1:.3f} ({avg_bert_f1 * 100:.1f}%)")
    print(f"Total examples processed: {total}")

def main():
    """Main function to run generative evaluation."""
    print("🎯 Bengali Riddle Generative Evaluation - DeepSeek-R1:7b")
    print("=" * 60)
